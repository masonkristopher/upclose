...
const ChatSend: FC<ChatSendProps> = ({ user, socket }) => {
  ...
  return (
    <div className="relative p-3 border rounded bg-gray-200 h-chat">
      <div className="overflow-y-auto h-40">
        {messages.map((msg: any) => {
          const key = `${msg.messageCount}-${msg.socketId}`;
          return (
            <div key={key} className="relative leading-none">
      ...
      <div className="absolute bottom-0 py-2 w-full">
        <input id="chat-input" value={message} onKeyPress={onKeyPress} onChange={(e) => setMessage(e.target.value)} type="text" className="appearance-none shadow float-left w-chat-input bg-gray-100 text-gray-700 border border-gray-400 rounded py-2 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" />
        <button onClick={(e) => sendChat(e)} type="button" className="float-left bg-white hover:bg-pink-100 text-gray-800 font-semibold p-1 mx-2 border border-gray-400 rounded-full shadow">
