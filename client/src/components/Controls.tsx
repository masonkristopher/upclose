...
const ControlPanel: FC<ControlPanelProps> = ({ userVideo }): ReactElement => {
  ...
  // returns mic icon, depending on if audio is on or off
  const micSVG = () => {
  ...
  // returns video icon, depending on if video is on or off
  const vidSVG = () => {
  ...
  return (
    <div className="justify-center z-40 inline-block">
      {/* Pause Video Button */}
      <button
        onClick={pauseVideo}
        type="button"
